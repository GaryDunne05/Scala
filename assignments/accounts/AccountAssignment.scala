...
object AccountAssignment extends App {
  ...
  case class accountAggregations(
                                  customerId : String,
                                  numberAccounts : Int,
                                  totalBalance : Long,
                                  averageBalance : Double
                                )
  case class groupedAccounts(
                           customerId: String,
                           accounts: Seq[AccountData]
                         )
  case class combinedClasses(
                                customerId : String,
                                accounts: Seq[AccountData],
                                numberAccounts : Int,
                                totalBalance : Long,
  ...
  //group dataset by customer ID
  val accountsByKeyDS = accountDS.groupByKey(x => x.customerId)
  //group accounts by customer ID
  val groupedAccountsDS = accountsByKeyDS.mapGroups((key, accounts) => groupedAccounts(key, accounts.toSeq))
  //get aggregations for each customer
  val accountAggregationsDS = accountsByKeyDS.agg(typed.count(_.accountId), typed.sum(_.balance), typed.avg(_.balance))
    .map(aggregations => accountAggregations(aggregations._1, aggregations._2.toInt, aggregations._3.toLong, aggregations._4)  )
  //combine grouped accounts and the aggregations datasets
  val combinedColumnsDS = groupedAccountsDS
    .joinWith(accountAggregationsDS, accountAggregationsDS("customerId") === groupedAccountsDS("customerId"),"left_outer")
    .map{case (accounts,aggregations) => combinedClasses(
      aggregations.customerId,
      accounts.accounts,
      aggregations.numberAccounts,
      aggregations.totalBalance,
      aggregations.averageBalance)
    }

  val customerAccountOutputDS = customerDS
    .joinWith(combinedColumnsDS, combinedColumnsDS("customerId") === customerDS("customerId"),"left_outer")
    .map{
      //case the customer has no accounts - set accounts as empty and aggregations as 0
      case (customer, null)
            => CustomerAccountOutput(customer.customerId, customer.forename, customer.surname, Seq.empty[AccountData], 0, 0, 0)
      //case the customer has accounts
      case (customer, accounts )
            => CustomerAccountOutput(accounts.customerId, customer.forename, customer.surname,
              accounts.accounts, accounts.numberAccounts, accounts.totalBalance,accounts.averageBalance)
    }
    .orderBy("customerId")
